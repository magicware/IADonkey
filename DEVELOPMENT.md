...
- <kbd>Alt + Enter</kbd>: Zobrazit **Subpoložky (`options`)** položky (např. instance MagicGate, podsložky se samostatným vyhledáváním).
- <kbd>Ctrl + Enter</kbd>: Okamžité rychlé spuštění 1. subpoložky položky.
- <kbd>Escape</kbd>: Vyčistit hledání / návrat ze subpoložek a akcí / zavřít okno.
...
- **Podpora pro vzdálené JSON a API zdroje**:
  ...
  - **Příklad struktury JSONu**:
    ```json
    [
      {
        "name": "Produkční server 01",
        "location": "https://admin.example.com",
        "icon": "dns",
        "info": {
          "Server": "cz-prod-app-01",
          "IP Adresa": "192.168.1.50",
          "Databáze": "sql-prod-cluster.internal",
          "SSH Port": 2222,
          "Zálohování": "Denně ve 02:00"
        },
        "actions": [
          {
            "name": "Otevřít administraci",
            "action": "open",
            "location": "https://admin.example.com"
          },
          {
            "name": "Kopírovat SSH spojení",
            "action": "copy",
            "location": "ssh admin@192.168.1.50 -p 2222",
            "icon": "terminal"
          }
        ]
      }
...
### H. Barevné schéma a akcenty aplikace
- **Hlavní barva (`primaryColor`)**: Konfigurovatelná v Nastavení -> Vzhled (výchozí `#6366f1` Indigo). Ovlivňuje primární tlačítka, aktivní taby, přepínače, fokusy a vybraný řádek ve Spotlightu.
- **Barva akcí a informací (`actionsColor`)**: Konfigurovatelná v Nastavení -> Vzhled (výchozí `#a855f7` Fialová). Ovlivňuje nabídku Akcí (Shift+Enter), štítky akcí a dialog pro klonování repozitářů.
- **Semaforové stavové barvy**: Zůstávají fixní (červená = chyby/smazat, žlutá/oranžová = varování/sync, zelená = úspěch/kalkulačka/zkopírováno).
- Obě dynamické barvy jsou řízeny přes `src/utils/theme.ts` (`applyPrimaryColor` a `applyActionsColor`) a aplikují se v reálném čase bez nutnosti restartu aplikace.

### I. Závazné pravidlo pro aktualizaci dokumentace zdrojů dat (Jak na zdroje dat)
- **Kdykoliv dojde k rozšíření JSON modelu (`LauncherItem`, `LauncherAction`, `DataSource`), přidání nové akce (`action`) či systémové integrace (`settings`):**
  - Je **povinné** aktualizovat průvodce **"Jak na zdroje dat" (`src/components/DataSourcesGuideModal.tsx`)**.
  - Zapsat nový atribut do přehledu schématu, popsat jeho chování a přidat funkční ukázku do kopírovatelných JSON snippetů.
  - Tlačítko k tomuto průvodci je trvale dostupné nahoře v záložce *Nápověda a zkratky* v Nastavení aplikace.
